...
(defn eq-first [n [a b]]
  (or (zero? n)
      (and (= (rem a 2) (rem b 2))
           (eq-first (dec n) [(quot a 2) (quot b 2)]))))

(defn part-1
  "day 15 part 1 solution"
  []
  (->> (get-input)
    next-pair (iterate next-pair) (take 40000000) (filter #(eq-first 16 %)) count))
...
(defn part-2
  ...
  (let [[a b] (get-input)
    ...
    (->> (map vector as bs) (take 5000000) (filter #(eq-first 16 %)) count)))
